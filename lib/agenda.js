...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    this._collection.findAndModify({_id: id, name: props.name}, {}, update, {new: true}, processDbResult );
    ...
    this._collection.findAndModify(
      {
        ...
        $or: [
          {lockedAt: null, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }},
          {lockedAt: {$exists: false}, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }},
          {lockedAt: {$lte: lockDeadline}, disabled: { $ne: true }}
        ]
      },
      {'priority': -1},  // sort
